...
jQuery.fn.slugIt = function(options) {
	...
	jQuery(this).bind(defaults.events, function() {
		...
		if (defaults.type == 'keys') {
			slug = slug.replace(/[^a-z0-9\-\s\,]/g, '-');		// remove unneeded chars
			slug = slug.replace(/^[\-\s\,]+/g, '');				// trim leading dashes/spaces/commas
			slug = slug.replace(/\s*\-+\s*/g, '-');				// convert multiple dashes
			slug = slug.replace(/\-+/g, '-');					// convert multiple dashes
			slug = slug.replace(/\s+/g, ' ');					// convert multiple spaces
			slug = slug.replace(/[\-\s\,]?,[\-\s\,]*/g, ', ');	// convert multiple commas
		} else {
			slug = slug.replace(/[^-a-z0-9\s]/g, '-');			// remove unneeded chars
			slug = slug.replace(/^[-\s]+|[-\s]+$/g, '');		// trim leading/trailing dashes/spaces
			slug = slug.replace(/[-\s]+/g, '-');				// convert multiple dashes/spaces
		}
