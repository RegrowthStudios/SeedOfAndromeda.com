...
// Adapted by drt24 (Daniel Thomas) to add the optional $wgAuthXenforoMailDomain and remove hardcoding
...
$wgExtensionCredits['other'][] = array(
		'name' => 'XenForo Authentication',
		'version' => '1.1.4',
		'author' => array( 'Jesse Leskinen', 'Otheus Shelling', 'Rusty Burchfield', 'James Kinsman', 'Daniel Thomas', 'Ian Ward Comfort' ),
		'url' => 'https://www.mediawiki.org/wiki/Extension:AutomaticREMOTE_USER',
		'description' => 'Use XenForo for authentication',
);
...
$wgAuthXenforoSDK = new XenForoSDK();
$wgAuthXenforoUser = $wgAuthXenforoSDK->getUser();
if(in_array("timezone", $wgAuthXenforoUser)){
	$wgLocaltimezone = $wgAuthXenforoUser["timezone"];
}




$wgExtensionFunctions[] = 'Auth_xenforo_hook';
$wgHooks['UserLogout'][]      = 'Auth_xenforo_onUserLogout';
$wgHooks['PersonalUrls'][] = 'Auth_xenforo_onPersonalUrls';
$wgHooks['SpecialPage_initList'][]='Auth_xenforo_disableLogin';

function Auth_xenforo_disableLogin(&$list) {
	unset($list['Userlogin']);
	return true;
}

function Auth_xenforo_onPersonalUrls( array &$personal_urls, Title $title, SkinTemplate $skin ) {
	global $wgAuthXenforoSDK, $wgAuthXenforoUser;
	$visitor = XenForo_Visitor::getInstance();
	$redirect_url = $title->getFullURL();
	foreach($personal_urls as $key => $val){
		switch($key){
		case "login":
		case "anonlogin":
			$personal_urls[$key]['href'] = XenForo_Link::buildPublicLink('canonical:login', $wgAuthXenforoUser, array('_xfToken' => $visitor['csrf_token_page'], 'redirect' => $redirect_url));
		break;
		case "logout":
			$personal_urls[$key]['href'] = XenForo_Link::buildPublicLink('canonical:logout', $wgAuthXenforoUser, array('_xfToken' => $visitor['csrf_token_page'], 'redirect' => $redirect_url));
		break;
		case "createaccount":
			$personal_urls[$key]['href'] = XenForo_Link::buildPublicLink('canonical:register', $wgAuthXenforoUser, array('_xfToken' => $visitor['csrf_token_page'], 'redirect' => $redirect_url));
		break;
...
* @param object $user 
*/ 
 function Auth_xenforo_onUserLogout(&$user) 
{ 
	global $wgAuthXenforoSDK;
	$wgAuthXenforoSDK->logout();
	return true; 
} 
 ...
 */
function Auth_xenforo_hook() {
	global $wgUser, $wgRequest, $wgAuthXenforoDomain, $wgAuth, $wgAuthXenforoSDK, $wgAuthXenforoUser;
	...
	if ( !$wgAuthXenforoSDK->isLoggedIn() ) {
		if(!$user->isAnon()){
			$user->doLogout();
		}
		return;
	...
	if ( !$user->isAnon() ) {
		if ( $user->getName() == $wgAuth->getCanonicalName( $wgAuthXenforoUser ['username'] ) ) {
	...
	do {
		...
		$params = new FauxRequest( array(
			'wpName' => $wgAuthXenforoUser ['username'],
			'wpPassword' => '',
			'wpDomain' => '',
			'wpLoginToken' => $token,
			'wpRemember' => ''
		...
		if ( $result != LoginForm::SUCCESS && $result != LoginForm::NEED_TOKEN ) {
			error_log( 'Unexpected XenForo authentication failure. Login Error was:' . $errormessage );
		}
		$trycount++;
	} while ( $tryagain );
...
class Auth_xenforo extends AuthPlugin {
	/**
	...
	public function authenticate( $username, $password ) {
		...
		return $wgAuthXenforoSDK->validateLogin($username,$password) == true;
	}
	...
	global $wgAuthXenforoUser;
		// We only set this stuff when accounts are created.
		$user->setEmail( $wgAuthXenforoUser['email'] );
	...
	public function initUser( &$user, $autocreate = false ) {
		...
		if ( $wgAuthXenforoUser['default_watch_state'] == "watch_email" ) {
			$user->setOption( 'enotifwatchlistpages', 1 );
		}
		
		if ( $wgAuthXenforoUser['email_on_conversation'] == 1 ) {
	...
	public function modifyUITemplate( &$template, &$type ) {
		...
		$template->data['message'] = "You need to use the forums to log in!"; 
