...
<?php
				...
				header ( "Location: /" . $pageurl . "?imageid=" . $id );
            } else {
				echo '
                <div class="row clearfix">
                    <div class="header"><h1>Images Manager</h1></div>
                ...
                if ($canmanageimages) {
					...
					while ( $row = $query->fetch() ) {
						echo '<li>' . $row ["title"] . ' - <a href="/' . $pageurl . '?imageid=' . $row ["id"] . '">Edit</a> - <a onclick="return confirmAction(\'Are you sure you wish to delete this image?\');" href="/' . $pageurl . '?imageid=' . $row ["id"] . '&delete=1">Delete</a></li>';
					}
					echo "</ul>";
                }
                echo '
