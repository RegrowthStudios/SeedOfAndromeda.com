...
if($id != "")
{
	$result = mysql_query("SELECT * FROM devlog WHERE id = '$id'");
    ...
    <TD width="926" valign="top">
        ...
        <TD height="100%">
            ...
            <TD valign="top" style="border-right: 1px solid black;">
				...
				<TR>
                <TD onClick="window.location.href='/master/master_devlog.php'" style="font-weight: bold; cursor: pointer; cursor: hand; border-bottom: 1px solid #666666; padding-left: 5px;" height="30" width="100%"><font style="font-size: 12px; font-family: arial;">Developer Log</font></TD>
                </TR>
				<TR>
            ...
            <TD width="804" height="100%" valign="top">
                ...
                <TR><form action="/master/master_devlog_function.php" method="post">
                <TD valign="top" style="padding: 15px;">
                Title: <input type="text" name="title" value="<?php echo $title; ?>" style="width: 300px;"><BR><BR>
				...
				Posts:
					...
					<?php
					$result = mysql_query("SELECT * FROM devlog ORDER BY date_created DESC");
					while($row = mysql_fetch_array($result))
					{
					echo'<TR>
					<TD>'.$row['id'].'</TD>
					<TD><a href="/master/master_devlog.php?id='.$row['id'].'">'.$row['title'].'</a></TD>
					<TD>'.$row['date_created'].'</TD>
					<TD><a href="/master/master_devlog_delete.php?id='.$row['id'].'">DELETE</a></TD>
					</TR>';
					}					
					?>
					</table>  
